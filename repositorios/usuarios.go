...
func (repositorios Usuarios) Seguir(usuarioId, seguidorId uint64) error {
	statement, erro := repositorios.db.Prepare("insert ignore into seguidores (usuario_id, seguidor_id) values (?,?)")
	if erro != nil {
		return erro
	}
...
func (repositorios Usuarios) BuscarSeguidores(usuarioId uint64) ([]modelos.Usuario, error) {
	linhas, erro := repositorios.db.Query(`
		select u.id, u.nome, u.nick, u.email, u.criadoEm from usuarios u inner join seguidores s on u.id = s.seguidor_id 
...
func (repositorios Usuarios) BuscarSeguindo(usuarioId uint64) ([]modelos.Usuario, error) {
	linhas, erro := repositorios.db.Query(`
